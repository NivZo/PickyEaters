...
public static class HintSystemAStar
{
    // State for A* search
    private class SearchState
    {
        ...
        // Track which food types have "last" items remaining
        ...
        public override bool Equals(object obj)
        {
            ...
            // Compare food states
            if (!FoodState.SetEquals(other.FoodState))
                return false;
                
            // Compare last food positions
            if (LastFoodPositions.Count != other.LastFoodPositions.Count)
                return false;
                
            foreach (var pair in LastFoodPositions)
            {
                if (!other.LastFoodPositions.TryGetValue(pair.Key, out Vector2I otherPos) || 
                    otherPos != pair.Value)
                    return false;
            }
